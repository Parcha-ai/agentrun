...
3. Copy `examples/support-answer-config.example.mjs` to `support.config.mjs`. Implement `runEffect` for `help.search` and `runNode` for the existing agent. Register read-only `support_read` with the appropriate account scope. Forward the full agent request, including tools, schema, review, and cancellation. Keep secrets out of tracked files. A `.env` file needs an explicit loader.
