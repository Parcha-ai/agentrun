...
export class WorkflowStoreError extends Error {
  constructor(readonly code: 'invalid_name' | 'invalid_revision' | 'invalid_workflow' | 'unsafe_path' | 'not_found' | 'invalid_record' | 'ambiguous_revision', message: string) {
...
export class WorkflowStore {
  ...
  private async latest(name: string): Promise<string | undefined> {
    ...
    try {
      ...
      if (!stat.isFile() || await realpath(path) !== path) throw unsafe();
      if (stat.size !== 65) throw new WorkflowStoreError('invalid_record', 'Saved workflow latest revision is invalid.');
      const value = await handle.readFile('utf8');
      if (!/^[a-f0-9]{64}\n$/.test(value)) throw new WorkflowStoreError('invalid_record', 'Saved workflow latest revision is invalid.');
      return value.slice(0, -1);
    } finally { await handle.close(); }
  }

  /** All revisions, newest first; the active revision leads within a timestamp tie. */
  async list(): Promise<WorkflowRevision[]> {
    ...
    for (const name of await readdir(directory)) {
      ...
      if (!child) continue;
      const active = await this.latest(name);
      if (active) latest.set(name, active);
      for (const file of await readdir(child)) {
        // Unpublished files survive an interrupted write and never become revisions.
        if (/^\.pending-[a-f0-9-]+$/.test(file) || file === 'latest') continue;
    ...
    return revisions.sort((a, b) => b.createdAt.localeCompare(a.createdAt) || a.name.localeCompare(b.name)
      || Number(b.digest === latest.get(b.name)) - Number(a.digest === latest.get(a.name)) || a.digest.localeCompare(b.digest));
  }
  ...
  async load(name: string, digest?: string): Promise<StoredWorkflow> {
    ...
    if (latest) return this.read(name, latest);
    // Revisions saved before the latest pointer was introduced remain readable.
    const revisions = (await this.list()).filter(item => item.name === name);
    const revision = revisions[0];
    if (!revision) throw new WorkflowStoreError('not_found', 'Saved workflow not found.');
    if (revisions[1]?.createdAt === revision.createdAt)
      throw new WorkflowStoreError('ambiguous_revision', 'Saved workflow revisions share a timestamp. Load by complete digest or save the intended revision again.');
  ...
  async save(name: string, value: unknown): Promise<StoredWorkflow> {
    ...
    // This rename is the linearization point for "load by name", even when
    // different revisions have identical millisecond timestamps.
    const pointerTemporary = join(directory, `.pending-${randomUUID()}`);
    const pointer = await open(pointerTemporary, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | constants.O_NOFOLLOW, 0o600);
    try { await pointer.writeFile(`${digest}\n`); await pointer.sync(); }
    finally { await pointer.close(); }
    if (await this.directory(name) !== directory) throw unsafe();
    await rename(pointerTemporary, join(directory, 'latest'));
    const updatedParent = await open(directory, constants.O_RDONLY | constants.O_NOFOLLOW);
    try { await updatedParent.sync(); } finally { await updatedParent.close(); }
    return saved;
  }
}
