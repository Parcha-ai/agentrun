...
A host's examples show direct tool results passed to a judge unchanged, followed by
deterministic probability projection in code. Adapt the tool and its return schema
to the tools the host actually names; an agent locator can produce the references
