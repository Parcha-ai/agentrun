...
3. Call `agentrun` with `action: "run"` and the input. Do not include a workflow in the run call. Execute the inspected definition. If code or mutating tools are needed, leave the inspected draft and input ready for the user's trusted-run command named in the host addendum instead.
