...
- Validation now refuses an `escalate.when` path or key that no input or earlier node produces, and a parallel branch that reads a key only a sibling branch writes. Both could never hold at runtime.
