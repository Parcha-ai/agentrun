...
- Breaking for Pi SDK callers, with no forwarding exports (prerelease): `@parcha/agentrun-pi` no longer exports `authorWorkflow`, `AUTHOR_CONTRACT` or `loadPiAuthorSkillBundle`. Import `authorWorkflow` from `@parcha/agentrun-dsl` and pass `runNode: createPiRunner({ ...options, tools: [], maxSubmissions })`. The Pi package registers the dsl skill rendered with its own host addendum (`PI_HOST_ADDENDUM`), built into `dist/skills/author`, and `describe` returns `authoring.language` and `authoring.host` beside the guides.
